...
TEST(QPyModule, CallAsyncWithVariantListRunsAndReturns) {
    ...
    auto f = m->callAsync("test_func", QMetaType::Double, std::move(args)).value();
    f.waitForFinished();
    auto res = f.resultAs<double>(0);
    EXPECT_EQ(res, 6.0);
}
...
TEST(QPyModule, CallAsyncReturnVoid) {
    ...
    auto f = m->callAsync("test_func", QMetaType::Void, std::move(args)).value();
    f.waitForFinished();
...
TEST(QPyModule, CallAsyncReturnVoid2) {
    ...
    QVariant y = 3.5;
    QVariantList args = {x, y};
    auto f = m->callAsync("test_func", "void", std::move(args)).value();
    f.waitForFinished();
    EXPECT_EQ(f.state(), qtpyt::QPyFutureState::Finished);
    EXPECT_EQ(f.resultCount(), 0);
}
