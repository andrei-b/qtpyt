...
namespace qtpyt {
    ...
    py::memoryview qByteArrayToPyArray(const QVariant &ba) {
        QByteArray *keeper;
        keeper = new QByteArray(ba.value<QByteArray>());
    ...
    py::memoryview qPySharedByteArrayToMemoryView(const QVariant &sba) {
        QPySharedByteArray *keeper;
        keeper = new QPySharedByteArray(sba.value<QPySharedByteArray>());
        py::capsule cap(keeper, [](void *p) { delete static_cast<QPySharedByteArray *>(p); });
        ...
        // readonly=1, flags=contiguous
        if (PyBuffer_FillInfo(&view, exporter, (void *) keeper->data(), (Py_ssize_t) keeper->size(),
                              keeper->isReadOnly(), PyBUF_CONTIG_RO) != 0) {
            Py_DECREF(exporter);
            throw py::error_already_set();
        }
            ...
            for (auto item: seq) {
                ...
                if (!v.has_value()) {
                    throw std::runtime_error(
                        "sequenceToQVariantList: Failed to convert item to QVariant in QVariantList conversion");
                }
                list.append(v.value());
            }
            return list;
    ...
    auto dictToQVariantMap(const py::object &obj) {
        ...
        if (py::isinstance<py::dict>(obj)) {
            ...
            for (auto item: dict) {
                ...
                if (!value.has_value()) {
                    throw std::runtime_error(
                        "dictToQVariantMap: Failed to convert value to QVariant in QVariantMap conversion");
                }
    ...
    auto dictToQVariantHash(const py::object &obj) {
        ...
        if (py::isinstance<py::dict>(obj)) {
            ...
            for (auto item: dict) {
                ...
                if (!value.has_value()) {
                    throw std::runtime_error(
                        "dictToQVariantHash: Failed to convert value to QVariant in QVariantHash conversion");
                } else {
    ...
    const static std::unordered_map<QByteArray, ValueFromStringFunc> specializedStringConverters = {
        ...
        {
            makeNormalName("QDateTime"),
            [](const QString &val) { return QVariant::fromValue(QDateTime::fromString(val)); }
        },
    ...
    void registerSharedByteArray() {
        int id = qRegisterMetaType<QPySharedByteArray>("QPySharedByteArray");
        const auto normName = makeNormalName("QPySharedByteArray");
        specializedPyObjectConverters.insert({
            normName, [](const py::handle &obj) {
                if (!obj || obj.is_none()) {
                    return QVariant::fromValue(QByteArray());
                }
                if (py::isinstance<qtpyt::QPySharedByteArray>(obj)) {
                    const auto rba = obj.cast<qtpyt::QPySharedByteArray>();
                    return QVariant::fromValue(rba);
                }
                throw std::runtime_error("Expected a QPyReadOnlyByteArray for QByteArray conversion");
            }
        });
        addFromQVariantFunc(id, qPySharedByteArrayToMemoryView);
        addMetatypeVoidPtrToPyObjectConverterFunc(static_cast<QMetaType::Type>(id), [](const void *v) {
            const auto sba = *static_cast<const QPySharedByteArray *>(v);
            return qtpyt::qPySharedByteArrayToMemoryView(QVariant::fromValue(sba));
        });
    }
    ...
    QVariantList pySequenceToVariantList(const py::iterable &seq) {
        ...
        for (auto item: seq) {
            ...
            if (!v.has_value()) {
                throw std::runtime_error(
                    "pySequenceToVariantList: Failed to convert item to QVariant in QVariantList conversion");
            }
    ...
    QVariantMap pyDictToVariantMap(const py::dict &d) {
        ...
        for (auto [fst, snd]: d) {
            ...
            if (!v.has_value()) {
                throw std::runtime_error(
                    "pyDictToVariantMap: Failed to convert value to QVariant in QVariantMap conversion");
            }
    ...
    template<typename T>
    py::object podToPyObject(const void *v) {
    ...
    template<typename T>
    py::object podArrayToPyArray(const void *data, size_t count) {
    ...
    // C++
    template<typename T>
    py::object podVectorToMemoryView_move(std::vector<T> &&vec) {
        static_assert(std::is_trivial_v<T>, "T must be a POD type");
    ...
    template<typename T>
    py::object podVectorToMemoryView_shared(const std::vector<T> &src) {
        static_assert(std::is_trivial_v<T>, "T must be a POD type");
