...
namespace qtpyt {
    QPyFuture::QPyFuture(QPyModule module, const QString& functionName, const QByteArray& returnType,
                         QVariantList&& arguments) {
        m_impl = std::make_shared<QPyFutureImpl>(std::move(module), functionName, returnType, std::move(arguments));
    }

    QPyFuture::QPyFuture(QPyModule module, QString functionName, const QByteArray& returnType,
        const QVector<int>& types,  void** a) {
        m_impl = std::make_shared<::QPyFutureImpl>(std::move(module), std::move(functionName), returnType, types, a);
    }
